...
if nargin >=5
    if (length(size(vlb))==2 & size(vlb,2)==1)
        vlb = vlb';
    end
end
if nargin >=6
    if (length(size(vub))==2 & size(vub,2)==1)
        vub = vub';
    end
end
...
fcalls=0; dv=Inf;
if prnt
 fprintf('%7s %10s %12s\n','f-COUNT','FUNCTION','x-TOL'); %uncomment pls
end 
i=0;
while (i<lx)&(dv>=tolx)
...
[fmin,fconstr]=feval(funfcn,x0,varargin{:}); % Calls user function

f_argz = varargin{1};
A_alt = f_argz{13};
b_alt = f_argz{14};
W_G_alt = f_argz{15};
G_alt = W_G_alt*(A_alt*(x0')-b_alt);

BND_PEN = f_argz{5};  % Upper bound penalty
PROP_PEN = f_argz{6}; % Prop usage penalty
LBND_CONSTR = vlb-x1; % Negative values means it violated lower bound
UBND_CONSTR = x1-vub; % Negative values means it violated upper bound
TNUM_PEN = f_argz{7}; % Penalty per additional thruster over max allowed
TNUM_LIM = f_argz{8}; % Number of thrusters before penaly incurred

% This penalizes the cost for each thruster over TNUM_LIM that is used
if numel(x(x > 0.00001)) > TNUM_LIM
fmin = fmin*TNUM_PEN*(numel(x(x > 0.00001))-TNUM_LIM);
end
% Should heavily penalize cost if using x outside provided bounds or
% violating equality constraints
fmin = PROP_PEN*fmin + ... % Prop usage penalty and thruster count penalty
1e8*sum(abs(LBND_CONSTR(LBND_CONSTR > -1e-10)),2) + ... % Lower bound violation penalty
BND_PEN*sum(UBND_CONSTR(UBND_CONSTR > 0),2) + ... % Upper bound penalty
sum(abs(fconstr),2) + sum(abs(G_alt)); % Target force and torque penalty
